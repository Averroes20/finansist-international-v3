        ...
        <div className="flex flex-row whitespace-nowrap animate-slider" style={{ willChange: 'transform' }}>
          {softwareList.map((item, index) => (
            <div key={`software-${index + 1}`} className="flex flex-row items-center min-w-fit mx-5 space-x-2">
              <Image src={item.value} alt={item.label} width={1000} height={1000} className="w-[45px] md:w-[50px] object-contain" />
              <p className="text-lg">{item.label}</p>
            </div>
          ))}
        </div>
      </div>
      <div className="absolute -right-4 top-0 h-full border-r-slate-900 w-1/5 md:w-1/3 z-[5] bg-gradient-to-l from-white to-transparent" />
    </div>
  );
};
