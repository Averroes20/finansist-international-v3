...
const Services = () => {
  ...
  const [servicePrice, setServicePrice] = useState<{ id: number; code: string; sort: number; fee: number; }[]>([]);
  ...
  return (
    <section className="max-w-screen-xl mx-auto min-h-screen py-5 px-5 md:px-0 md:my-10">
      ...
      <div className="flex items-center justify-center gap-4 my-5">
        <button
          onClick={onToggle}
          className="p-2 flex items-center gap-3 rounded-md bg-white text-white font-bold"
        >
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-x-0 gap-y-10 md:gap-x-6 md:gap-y-20 pb-6">
          {items?.map((service, index) => (
            <div key={`${service.link}-${index + 1}`} id={service.link} className={`flex flex-col scroll-mt-28`}>
              <ServiceCard service={service} price={servicePrice[index]?.fee} isAnnual={isAnnual} code={select.name} lang={language} />
            </div>
          ))}
        </div>
      </div>
